...
OnOffApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::OnOffApplication")
	...
	.AddAttribute ("FlowId",
				   "Packet flow id",
				   UintegerValue (0),
				   MakeUintegerAccessor (&OnOffApplication::m_fid),
				   MakeUintegerChecker<UintegerValue> (0,1000))
...
void OnOffApplication::SendPacket ()
{
  ...
  packet->AddByteTag(myflowIdTag);

  DelayJitterEstimation mytxtime;
  mytxtime.PrepareTx (packet);
  //DelayJitterEstimationTimestampTag myTimeTag;
 // packet->AddByteTag(myTimeTag);
  ...
  if (InetSocketAddress::IsMatchingType (m_peer))
    {
      NS_LOG_INFO ("At time " << Simulator::Now ().GetSeconds ()
                   ...
                   << " total Tx " << m_Sent[fid.Get()] << "packets, " << m_totBytes << " bytes"<<" Flowid: "
                   << fid.Get());
    }
  else if (Inet6SocketAddress::IsMatchingType (m_peer))
    {
      NS_LOG_INFO ("At time " << Simulator::Now ().GetSeconds ()
                   ...
                   << " total Tx " << m_totBytes << " bytes"<<" Flowid: "
                   << fid.Get());
    }
  m_lastStartTime = Simulator::Now ();
  m_residualBits = 0;
