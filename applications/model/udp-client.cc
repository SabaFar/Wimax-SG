...
UdpClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::UdpClient")
   ...
   .AddAttribute ("FlowId",
				   "Packet flow id",
				   UintegerValue (0),
				   MakeUintegerAccessor (&UdpClient::m_fid),
...
//      	NS_LOG_INFO ("UDP_FLOWS:Flow Id\tSent\tRecv\tReliable Received\tAvg. Delay\n");
//      		 for (int i=0; i< FLOW_NUM; i++)
//      		 {
//      			 //if (Recv[i] != 0)
//      			NS_LOG_INFO (i << "\t" << Sent[i] << "\t" << udpS->Recv[i] << "\t" << udpS->reliable_received[i] << "\t\t" << udpS->Delays[i].ToDouble(ns3::Time::MS)/float(udpS->Recv[i]) << std::endl ) ;
//      		 }
  ...
  else
    {
      NS_LOG_INFO ("Error while sending " << m_size << " bytes to "
                                          << peerAddressStringStream.str ()<< " Flowid: "
                                                  << fid.Get())  ;
    }
